...
@class TTNetImageView;

@protocol TTNetImageViewDelegate <NSObject>
@optional

/**
 * Called when the image begins loading asynchronously.
 */
- (void)netImageViewDidStartLoad:(TTNetImageView*)imageView;

/**
 * Called when the image finishes loading asynchronously.
 */
- (void)netImageView:(TTNetImageView*)imageView didLoadImage:(UIImage*)image;

/**
 * Called when the image failed to load asynchronously.
 * If error is nil then the request was cancelled.
 */
- (void)netImageView:(TTNetImageView*)imageView didFailLoadWithError:(NSError*)error;
 ...
 */
- (void)netImageView:(TTNetImageView*)imageView willSendARequest:(TTURLRequest*)requester;
